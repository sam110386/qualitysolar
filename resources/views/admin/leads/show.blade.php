    ...
    <div class="card-body">
        ...
        <div class="mb-2 row">
            ...
            <div class="col-sm-6">
                        ...
                        <tr>
                            ...
                            <td>
                                <select name="assigned_to_user_id" id="assigned_to_user_id" class="form-control">
                                    ...
                                    <option value="{{ $id }}" {{ (isset($lead) && $lead->assigned_to_user_id==$id) ?'selected' : '' }}>{{ $name }}</option>
                                    @endforeach
                                </select>
                            </td>
                        </tr>
                ...
                <form action="{{ route("admin.leads.updatelead", [$lead->id]) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('POST')
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td>
                                    <input type="text" id="quote" name="quote" class="form-control required" value="{{ old('quote', isset($lead) ? $lead->quote : '') }}">
                                    @if($errors->has('quote'))
                                    <em class="invalid-feedback">
                                        {{ $errors->first('quote') }}
                                    </em>
                            ...
                            <tr>
                                ...
                                <td>
                                    <input type="text" id="installation_date" name="installation_date" class="form-control required date" value="{{ old('installation_date', isset($lead) ? $lead->installation_date : '') }}">
                            ...
                            <tr>
                                ...
                                <td class="form-group">
                                    <select name="online_assessment_completed" id="online_assessment_completed " class="form-control required">
                                        <option value="0" {{ (isset($lead) && $lead->online_assessment_completed==0) ?'selected' : '' }}>No</option>
                                        <option value="1" {{ (isset($lead) && $lead->online_assessment_completed==1) ?'selected' : '' }}>Yes</option>
                                    </select>
                                </td>
                            </tr>
                ...
                @if($lead->status_id==5)
                <form action="{{ route("admin.leads.permitupdate", [$lead->id]) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('POST')
                    <table class="table table-bordered table-striped">
                        <thead>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="form-group">
                                    <select name="inspection_completed" id="inspection_completed " class="form-control  required">
                ...
                @if($lead->status_id==5)
                <form id="surveyupdate" action="{{ route("admin.leads.surveyupdate", [$lead->id]) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('POST')
                    <table class="table table-bordered table-striped">
                        <thead>
...
<script>
    $(function() {
        ...
        var agentVal;
        $("#assigned_to_user_id").change(function(e) {
            var newAgentVal = $(this).val();
            var con = confirm("Are you sure you want to update vendor for this record?");
            if (!con) {
                $(this).val(agentVal);
                return false;
            }
            ...
            $.ajax({
                url: "{{ route('admin.leads.assignvendor') }}",
                type: 'POST',
                dataType: "json",
                data: {
                    _token: _token,
