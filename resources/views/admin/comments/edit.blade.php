    ...
    <div class="card-body">
        <form action="{{ route("admin.comments.update", [$comment->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('lead_id') ? 'has-error' : '' }}">
                <label for="lead">{{ trans('cruds.comment.fields.lead') }}</label>
                <select name="lead_id" id="lead" class="form-control select2">
                    @foreach($leads as $id => $lead)
                        <option value="{{ $id }}" {{ (isset($comment) && $comment->lead ? $comment->lead->id : old('lead_id')) == $id ? 'selected' : '' }}>{{ $lead }}</option>
                    @endforeach
                </select>
